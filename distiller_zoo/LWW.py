...
class FeatureMatching(nn.ModuleList):
    def __init__(self, s_shapes, t_shapes):
        ...
        for s, t in itertools.product(s_shapes, t_shapes):
            self.append(nn.Conv2d(s[1], t[1], 1))

    def forward(self, g_s, g_t, weight, beta, loss_weight):

        matching_loss = 0.0
        for i, (f_s, f_t) in enumerate(itertools.product(g_s, g_t)):
            t_W = f_t.size(3)
            s_W = f_s.size(3)
            if t_W == s_W:
                diff = f_t - self[i](f_s)
            else:
                diff = F.interpolate(f_t, scale_factor=s_W / t_W, mode='bilinear' ) - self[i](f_s)
